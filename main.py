...
def load_system_prompt() -> str:
    """Fetch the current Mammoth OS system prompt from Supabase."""
    result: Any = (
        supabase.schema("mammoth")
        .table("mammoth_system")
        .select("value")
        .eq("key", "system_prompt")
        .execute()
    )
    if result.data and isinstance(result.data, list) and "value" in result.data[0]:
        return str(result.data[0]["value"])
    return ""

# Load the prompt once at startup
...
def mammoth_cli() -> None:
    ...
    while True:
        ...
        if prompt.lower().startswith("set personality:"):
            ...
            if not new_prompt:
                console.print(Text("\n⚠️  Please provide a new personality text after 'set personality:'", style="bold red"))
                continue

            supabase.schema("mammoth").table("mammoth_system").update({"value": new_prompt}).eq("key", "system_prompt").execute()
            SYSTEM_PROMPT = load_system_prompt()
            console.print(Text("\n✅  Mammoth OS personality updated successfully!", style="bold green"))
            continue
        ...
        if prompt.lower() == "reset personality":
            ...
            supabase.schema("mammoth").table("mammoth_system").update({"value": default_prompt}).eq("key", "system_prompt").execute()
            SYSTEM_PROMPT = load_system_prompt()
            console.print(Text("\n✅  Mammoth OS personality reset to default.", style="bold green"))
            continue
        ...
        if prompt.lower().startswith("set engine:"):
            ...
            if new_engine not in AGENTS:
                console.print(f"[bold red]⚠️ Unknown engine '{new_engine}'. Available:[/bold red] [yellow]{', '.join(AGENTS.keys())}[/yellow]")
                continue
            CURRENT_ENGINE = new_engine
            console.print(f"[bold green]✅ Inference engine switched to '{CURRENT_ENGINE}'.[/bold green]")
            continue

        # 🧠 View current inference engine
        if prompt.lower() == "view engine":
            console.print(f"[bold cyan]🦣 Current inference engine:[/bold cyan] [green]{CURRENT_ENGINE}[/green]")
            continue

        # 🧩 Help command
        if prompt.lower() == "help":
            console.print("[bold magenta]🦣 Mammoth OS CLI Commands:[/bold magenta]")
            console.print("[yellow]set engine:[/yellow] [green][deepseek|llama|hermes][/green]")
            ...
            console.print("[yellow]atlas progress:[/yellow] <user_id>: <lesson_id>: <status>")
        ...
        if prompt.lower().startswith("atlas lessons:"):
            ...
            if not lessons:
                console.print(f"[bold red]⚠️ No lessons found for module '{module}'.[/bold red]")
            else:
                console.print(f"[bold cyan]🦣 Lessons for module:[/bold cyan] [green]{module}[/green]")
                for lesson in lessons:
                    console.print(f"[yellow]- {lesson['lesson_title']}[/yellow]")  # type: ignore
            continue
        ...
        if prompt.lower().startswith("atlas progress:"):
            ...
            result = update_progress(user_id.strip(), lesson_id.strip(), status.strip())
        ...
        if prompt.lower().startswith("atlas visualize:"):
            ...
            from mammoth_os.visual_engine import render_progress, render_insight_summary
        ...
        try:
            supabase.schema("mammoth").table("mammoth_sessions").insert(data).execute()
